...
//            yOffset *= -1;
        ...
        while (tag != null && (!MathUtil.approximately(tag.rawPose.x, 0, 1) || !MathUtil.approximately(distance, Arm.Position.One.distanceFromBackdrop, 0.5))) {
            double powerY;
            double gainY = 0.016;
...
//            drive.turnTo(90, MathUtil.map(1, drive.getHeading(), 90, 0, 1));
    ...
    public void placePurplePixelByProp(PropPos pos) {
        ...
        Location left = new Location(startLocation.x - tile + 0.1, -tile, 90);
        if (startPos == StartPos.PIXEL_STACK) {
            left = new Location(startLocation.x - 0.1,
                    -tile, startLocation.angle);
